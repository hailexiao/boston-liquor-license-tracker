...
async function scrapeNextMeetingDate(url) {
    try {
        ...
        const currentYearElement = $("section#content") // Main page content
            ...
            .find(`.paragraphs-item-drawer .field.field-label-hidden div:contains('${currentYear}')` // Label element containing the current year
        )
        ...
        const meetingDates = currentDateStrings.map((dateString) => new Date(`${dateString}, ${currentYear}`));
        const nextMeetingDate = meetingDates.find((date) => date > currentDate) ?? null;
...
try {
    const nextMeetingDate = await scrapeNextMeetingDate("https://www.boston.gov/departments/licensing-board/licensing-board-information-and-members");
    if (nextMeetingDate) {
